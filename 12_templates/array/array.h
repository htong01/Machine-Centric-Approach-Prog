...
class Array{
  ...
  public:
	...
	Array(const Array& ar);               // copy constructer
	int size();
	Array& append(T x);
...
// template<typename T>
// Array<T>& Array<T>::append(Array<T> y)
// {
	// T* temp = (T *) malloc(sizeof(T) * (len+y.len));
	/*
	T* temp = new T[len + y.len];
	for (int i=0;i<len;i++){
		temp[i] = data[i];
	}
	for(int i=0;i<y.len;i++){
		temp[len+i] = y.data[i];                       // add the new array ata the end of the original one
	}
	delete []data;
	data = temp;
	len = len + y.len;
